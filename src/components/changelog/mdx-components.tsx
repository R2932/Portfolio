...
export function getMDXComponents(components?: MDXComponents): MDXComponents {
  return {
    img: ({ className, alt, src, width, height, ...props }: React.ComponentProps<"img">) => {
      ...
      return (
        <Image
          ...
          alt={alt || ""}
          width={typeof width === "string" ? Number.parseInt(width, 10) : width || 800}
          height={typeof height === "string" ? Number.parseInt(height, 10) : height || 600}
          src={src}
          {...props}
        />
      );
    ...
    Video: ({ className, ...props }: React.ComponentProps<"video">) => (
      <video className={cn("rounded-md border mb-5", className)} controls loop {...props} />
    ),
    Accordion,
    AccordionItem,
    AccordionTrigger,
